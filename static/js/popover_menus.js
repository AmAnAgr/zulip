...
export function initialize() {
    ...
    $("body").on("select", ".new_message_textarea, .message_edit_content ", (e) => {
        ...
        tippy(e.target, {
            ...
            placement: "top-start",
            trigger: "manual",
            // Shifting 10px to left places `B` above selection.
            // Extra 5px from top to get popover closer to text.
            offset: [caret_offset.left - 10, -1 * caret_offset.top + 5],
            arrow: false,
            showOnCreate: true,
            onHidden(instance) {
                instance.destroy();
            ...
            onShow(instance) {
                ...
                $popper.on("click", ".formatting_button", (e) => {
                    const format_type = $(e.currentTarget).find("[data-format]").data("format");
