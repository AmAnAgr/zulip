...
export function render_actions_remind_popover(element, id) {
    ...
    if (elt.data("popover") === undefined) {
        ...
        elt.popover("show");
        $(`.remind.custom[data-message-id="${CSS.escape(message.id)}"]`).flatpickr({
            enableTime: true,
            clickOpens: false,
            defaultDate: "today",
            minDate: "today",
...
export function open_message_menu(message) {
    ...
    $(".selected_message .actions_hover i").trigger("click");

    const popover_observer = new MutationObserver(async () => {
        if ($(".actions_popover").is(":visible")) {
...
export function any_active() {
    ...
    return (
        ...
        popover_menus.message_actions_popped() ||
        user_sidebar_popped() ||
